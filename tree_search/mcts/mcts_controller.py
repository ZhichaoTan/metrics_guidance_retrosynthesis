...
class MCTS:
    ...
    def build_tree(
        ...
        # while not self.done:
            ...
            if not chem_path and not rxn_path:
                print("not chem_path and not rxn_path")
    ...
    def _update_value(self, smiles: str):
        ...
        if rxn_data["type"] == "reaction":
            ...
            predecessor = next(self.tree.predecessors(smiles))
            chem_data = self.tree.nodes[predecessor]
            ...
            # if smiles == "CC(=O)C#N.S>>CC(=O)C(N)=S":
            #     import ipdb; ipdb.set_trace();
