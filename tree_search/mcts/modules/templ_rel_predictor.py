...
class TemplRelPredictor:
    ...
    def infer_all(self):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                preds.append(torch.topk(outputs, k=self.args.topk, dim=1)[1])
                # print("preds", preds[-1])
    ...
    def raw_to_processed(self):
        test_rxns_with_template_file = os.path.join(
            self.processed_data_path, "test_rxns_with_template.jsonl"
        )
        if not os.path.exists(test_rxns_with_template_file):
            raise FileNotFoundError(
                f"{test_rxns_with_template_file} is needed but does not exist! "
                f"Please re-run preprocessing on the test file to generate this file."
            )

        processed_test_file = os.path.join(self.processed_data_path, "processed_test.csv")
        if not os.path.exists(processed_test_file): 
            logging.info("Raw to Processed")

            # test_raw_csv = pd.read_csv(self.test_file)
            # new_ = test_raw_csv["rxn_smiles"].str.strip().str.split(">", expand=True)
            #
            # new_[0] = new_[0].map(utils.canonicalize_smiles)
            # new_[2] = new_[2].map(utils.canonicalize_smiles)

            # assert utils.canonicalize_smiles(
            #   test_raw_csv.loc[0, "rxn_smiles"].strip().split(">>")[0]
            # ) == new_.iloc[0, 1], "Did not flip"
            # new_.to_csv(processed_test_file, index=False)

            with (
                open(test_rxns_with_template_file, "r") as f,
                open(processed_test_file, "w") as of
            ):
                ...
                for line in f:
                    ...
                    r, _, p = rxn_with_template["rxn_smiles"].strip().split(">")
                    writer.writerow({
                        "0": utils.canonicalize_smiles(r),
                        "1": utils.canonicalize_smiles(p)
                    })
        else:
    ...
    def compile_into_csv(self):
        ...
        with open(test_rxns_with_template_file, "r") as f:
            clean_rxnsmi_phase = [
                json.loads(line.strip())["rxn_smiles"].strip()
                for line in f
            ]
        ...
        zipped = []
        for prod_smi, proposed_rcts_smi in zip(
                prod_smiles_phase,
