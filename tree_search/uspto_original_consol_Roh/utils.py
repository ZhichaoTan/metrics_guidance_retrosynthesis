...
def canonicalize_smiles(smiles: str, remove_atom_number: bool = True, remove_isotope: bool = False) -> str:
    ...
    return '.'.join(sorted(cano_smiles.split('.')))
...
def get_model(args, device) -> Tuple[nn.Module, Dict[str, Any]]:
    ...
    if args.load_from:
        ...
        state = torch.load(args.load_from, map_location=torch.device("cpu"), weights_only=False)
        ...
        args.n_templates = sum(1 for _ in open(os.path.join(args.processed_data_path, "templates.jsonl")))
        model = TemplRel(args)
...
def save_reactions_from_list(reactions: List[Dict[str, Any]], reaction_file: str):
    with open(reaction_file, "w") as of:
        for reaction in reactions:
            of.write(f"{json.dumps(reaction)}\n")

def load_reactions_as_list(reaction_file: str) -> List[Dict[str, Any]]:
    with open(reaction_file, "r") as f:
        reactions = [json.loads(line.strip()) for line in f]
