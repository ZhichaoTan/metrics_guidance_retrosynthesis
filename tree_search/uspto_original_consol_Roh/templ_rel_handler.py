...
class TemplRelHandler:
    ...
    def initialize(self, context):
        ...
        print(f"TorchServe gpu_id: {gpu_id}")
        ...
        if torch.cuda.is_available():
            ...
            if gpu_id is not None:
                ...
                # Default to GPU 0 when gpu_id is not specified but CUDA is available
                # This handles the case when using torchserve --models command line
                # where gpu_id may not be set even though GPU is available via Docker --gpus
                print(f"gpu_id not specified, defaulting to GPU 0: {torch.cuda.get_device_name(0)}")
        ...
        if next(self.model.parameters()).is_cuda:
            print(f"✅ Model is on GPU: {next(self.model.parameters()).device}")
        else:
            print(f"⚠️  Model is on CPU (GPU may not be available or gpu_id not set)")
