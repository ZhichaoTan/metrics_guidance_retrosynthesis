...
2. **Metric-guided retrosynthetic search**: Integrates metric calculations into the Monte Carlo Tree Search (MCTS) framework. The one-step model is consistent with [higherlev_retro](https://github.com/jihye-roh/higherlev_retro). **This implementation supports local deployment without requiring Docker or external servers**. We adapt corresponding modules from [ASKCOS](https://gitlab.com/mlpds_mit/askcosv2), and these modules can also be integrated into ASKCOS.
...
│   ├── ged_options.py                     # Command-line argument parser
...
│   │   ├── options.py                     # MCTS configuration options
│   │   ├── ged_weight_change.py           # GED weight scheduling functions
│   │   ├── utils_mcts.py                  # MCTS utility functions
│   │   └── modules/                       # Local API modules
│   │       ├── expand_one_api.py          # One-step expansion API (ASKCOS)
...
│   │       ├── templ_rel_predictor.py     # Template relevance predictor base
│   │       └── templ_rel_parser.py        # Template parser utilities
│   └── uspto_original_consol_Roh/         # Retrosynthesis model directory (extracted from .mar)
│       ├── model_latest.pt                # Trained model weights
│       ├── templates.jsonl                # Template library
│       ├── utils.py                        # Model utilities
│       ├── models.py                       # Model architecture definitions
│       └── misc.py                         # Miscellaneous utilities
...
│   ├── askcos_pickle.py                   # ASKCOS data utilities
│   ├── linear_tree_dfs.py                 # Tree traversal utilities
│   ├── sa_score_model/                    # SA_Score model files
│   │   └── fpscores.pkl.gz                # SA_Score fingerprint scores
│   └── sc_score_model/                    # SC_Score model files
│       └── model_1024bool.pickle          # SC_Score model weights
├── datastructs/                           # Data structure definitions
│   ├── datastructs.py                     # Core data structures
│   └── syn_tree.py                        # Synthesis tree data structure
├── data/                                  # Dataset files
│   ├── demo_targets.csv                   # Demo dataset (5 molecules)
│   └── uspto_190_targets.csv              # Full USPTO-190 dataset (190 molecules)
├── scripts/                               # Utility scripts
│   └── uspto_190.sh                       # Batch processing script for USPTO-190
├── utils/                                 # Utility functions
│   └── chem_utils.py                      # Chemistry utilities (SMILES canonicalization, etc.)
├── environment.yml                        # Conda environment configuration
├── test_metric_notebook.ipynb             # Jupyter notebook for metric testing
└── README.md                              # This file
```
...
The demo dataset is already included in `data/demo_targets.csv`, which can be replaced by your own dataset.
...
While this implementation provides local deployment for standalone use, the modules can also be integrated into the full [ASKCOS framework](https://gitlab.com/mlpds_mit/askcosv2) for more comprehensive retrosynthesis capabilities. To integrate with ASKCOS, you can follow the [ASKCOS guide](https://gitlab.com/mlpds_mit/askcosv2), and modify the code to use ASKCOS HTTP APIs instead of local APIs:
...
### Key Parameters for tree search
...
- Data structures for synthesis tree and chemistry utilities: https://github.com/jihye-roh/higherlev_retro
